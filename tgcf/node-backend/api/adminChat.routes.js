...
router.post('/send', adminAuthMiddleware, async (req, res) => {
    try {
        ...
        const messageId = await AdminChatModel.sendMessage(messageData);
...
// 获取聊天记录
router.get('/messages', adminAuthMiddleware, async (req, res) => {
    try {
...
// 获取与特定用户的聊天记录
router.get('/messages/:username', adminAuthMiddleware, async (req, res) => {
    try {
        const { username } = req.params;
        
        // 验证username参数不能为空
        if (!username || username === 'undefined') {
            return res.status(400).json({ success: false, error: '用户名参数不能为空' });
        }
