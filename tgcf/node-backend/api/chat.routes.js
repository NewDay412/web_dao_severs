...
router.post('/send', authMiddleware, async (req, res) => {
    try {
        const { sender_name, receiver_name, content, image_url, video_url } = req.body;
        
        // 验证必填字段
        if (!sender_name || !receiver_name) {
            return res.status(400).json({ success: false, error: '发送者和接收者不能为空' });
        }
        ...
        const messageId = await ChatModel.sendMessage(req.body);
...
router.get('/messages', authMiddleware, async (req, res) => {
    try {
        ...
        // 使用AdminChatModel获取所有消息，包括用户发送的
        const messages = await AdminChatModel.getMessages(parseInt(limit));
...
router.get('/messages/:username', authMiddleware, async (req, res) => {
    try {
        ...
        const { limit = 50 } = req.query;
        const messages = await AdminChatModel.getMessagesWithUser(username, parseInt(limit));
        res.json({ success: true, data: messages });
    } catch (error) {
...
router.post('/upload', authMiddleware, upload.single('file'), async (req, res) => {
    try {
        ...
        const messageId = await ChatModel.sendMessage(messageData);
        
        // 同时保存到管理员可查看的表中
        await AdminChatModel.sendMessage(messageData);
